                ...
                {displayOffers.map((offer) => {
                  ...
                  return (
                    <motion.div key={`${offer.id}-${offer.createdAt || ""}`} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} whileHover={{ scale: 1.005 }} onDoubleClick={(e) => { if (isMine || isAdmin) { e.stopPropagation(); setBgEditOfferId(String(offer.id)); setBgError(""); } }} className="tn-light relative w-full min-h-[110px] rounded-2xl bg-white/[0.04] border border-cyan-500/20 flex items-center gap-3 pr-2 pl-3 py-3 group shadow-[0_4px_24px_rgba(34,211,238,0.08)] hover:shadow-[0_0_32px_rgba(34,211,238,0.15)] transition-all">
                      {/* Banner BG */}
                      <div className="absolute inset-0 overflow-hidden rounded-2xl pointer-events-none"><div className="absolute inset-0 bg-[#070b1a]" /><div className="absolute right-0 top-0 bottom-0 w-[640px] max-w-[50%]"><div className="absolute inset-0" style={offerBgStyle} />{offerBg && <img src={offerBg} alt="" className="absolute inset-0 w-full h-full object-cover opacity-60" loading="lazy" decoding="async" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />}<div className="absolute inset-0 bg-gradient-to-r from-[#070b1a] via-[#070b1a]/70 to-transparent" /></div>{/* Region + Title + Prices Overlay */}<div className="absolute right-0 top-0 bottom-0 w-[640px] max-w-[50%] flex flex-col justify-between p-3 z-[5] pointer-events-none">{/* Top: Region + Title */}<div className="flex items-start justify-between gap-2"><h4 className="text-sm font-black tracking-widest text-white uppercase drop-shadow-[0_1px_6px_rgba(0,0,0,0.8)] truncate">{offer.title || `${offer.runsCount || 1}× Boost`}</h4>{offer.serverRegion && (<span className="shrink-0 px-2 py-0.5 rounded-full bg-violet-500/20 border border-violet-500/30 text-[9px] font-black tracking-widest text-violet-300 drop-shadow-[0_1px_4px_rgba(0,0,0,0.8)]">{String(offer.serverRegion).toUpperCase()}</span>)}</div>{/* Bottom: Prices */}{Number(offer.pricePerRun) > 0 && (<div className="flex items-center gap-3"><span className="px-2 py-0.5 rounded-md bg-black/50 text-[10px] font-black text-amber-300 drop-shadow-[0_1px_4px_rgba(0,0,0,0.8)]">Total: {(Number(offer.pricePerRun) * (offer.runsCount || 1)).toFixed(2)}M</span><span className="px-2 py-0.5 rounded-md bg-black/50 text-[9px] font-bold text-amber-200/80 drop-shadow-[0_1px_4px_rgba(0,0,0,0.8)]">{Number(offer.pricePerRun).toFixed(2)}M per run</span></div>)}</div></div>
