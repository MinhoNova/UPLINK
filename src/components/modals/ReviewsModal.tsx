...
export default function ReviewsModal({ isOpen, onClose, currentUserId, isAdmin = false }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <motion.div
        ...
        className="w-full max-w-lg bg-gradient-to-br from-[#0a0a16] to-black border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden max-h-[85vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="px-6 py-4 border-b border-white/5 flex items-center gap-3">
          <Star className="w-5 h-5 text-yellow-400 fill-yellow-400" />
          <div className="flex-1">
            <h2 className="text-sm font-black text-white uppercase tracking-widest">Reviews</h2>
        ...
        <div className="flex border-b border-white/5 px-4 gap-2">
          <button
            type="button"
            onClick={() => setTab("all")}
