...
export default function TierListClient() {
  return (
    <div className="min-h-screen bg-[#05050a] text-white">
      <ClassSidebar />
      <div className="relative z-10 lg:ml-[424px] pt-16 sm:pt-24 pb-16 px-4 sm:px-8 lg:px-12">

        {/* ─── Hero Banner ─── */}
        <div className="relative w-full max-w-[1600px] mx-auto aspect-[3/1] rounded-2xl overflow-hidden bg-black/40 mt-8">
          <Image
            ...
            sizes="100vw"
          />
          <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(5,5,10,0.05) 0%, rgba(5,5,10,0.75) 100%)" }} />
          <div className="absolute inset-0 flex flex-col justify-center items-center p-6 sm:p-10">
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white drop-shadow-2xl tracking-tight text-center" style={{ textShadow: "0 4px 40px #000" }}>
              Midnight <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00ffff] to-[#ff007f]">Season 1</span>
            </h1>
            <p className="text-xs sm:text-sm text-white/50 text-center mt-2 drop-shadow-lg font-medium tracking-wide" style={{ textShadow: "0 2px 10px #000" }}>
            ...
            {CLASS_ORDER.map((classId) => {
              ...
              return (
                <Link
                  ...
                  className="group relative block w-full aspect-[3/1] rounded-xl overflow-hidden bg-black/40 border border-white/[0.04] hover:border-white/[0.12] transition-all duration-300"
                >
                  <Image
