    ...
    <motion.nav animate={{ y: navVisible ? 0 : -96 }} className={`fixed top-0 w-full z-50 h-24 flex items-center ${theme === 'light' ? 'bg-white/50 text-black' : 'bg-transparent text-white'}`}>
      <div className="max-w-[1600px] mx-auto px-6 w-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <motion.button title={t('nav_autoApply')} onClick={() => {
              if (getUserTier(currentUserId) === "free") {
                window.dispatchEvent(new CustomEvent('show-toast', { detail: { msg: t('toast_autoLocked'), type: 'error' } }));
                return;
              }
              if (autoFeaturesLocked) {
                window.dispatchEvent(new CustomEvent('show-toast', { detail: { msg: t('toast_leaveRun'), type: 'error' } }));
                return;
              }
            ...
            <div className="relative" ref={langRef}>
              <motion.button title={t('nav_language')} onClick={() => setLangOpen(!langOpen)} className={`px-4 py-2 rounded-xl flex items-center gap-2 font-black uppercase text-[10px] tracking-widest transition-all bg-white/5 text-gray-400 hover:text-white border border-white/10 hover:bg-[#00ffff]/10 hover:border-[#00ffff]/30`}>
