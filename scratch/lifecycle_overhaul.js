...
content = content.replace(
    'lobbies.filter(l => l.ownerId !== currentUserId && l.accepted?.some((a:any) => a.applicantId === currentUserId)).map(l => (',
...
const modalButtonsRegex = /\{currentUserId === targetLobby\.ownerId && \(\n\s+<>\n\s+\{\(!targetLobby\.status \|\| targetLobby\.status === 'standby'\) && \([\s\S]+?Abort<\/button>\n\s+<>\n\s+\)\}/;
                               ...
                               <>
                                  ...
                                  {targetLobby.status === 'completed' && targetLobby.paymentProof && targetLobby.payoutStatus !== 'paid' && (
                                     <button onClick={() => { 
                                        ...
                                        // Update Stats for all accepted members
                                        const updatedUsers = [...registeredUsers];
                                        targetLobby.accepted.forEach((member: any) => {
...
const historyView = `
                    {activeArmoryTab === "history" && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        <h3 className="text-3xl font-black text-[#ff007f] mb-10 uppercase tracking-widest">Mission History</h3>
                        <div className="grid grid-cols-1 gap-6 max-h-[500px] overflow-y-auto pr-4 custom-scrollbar">
                           {lobbies.filter(l => l.status === 'completed' && (l.ownerId === currentUserId || l.accepted?.some((a:any) => a.applicantId === currentUserId))).length === 0 ? (
                              ...
                              lobbies.filter(l => l.status === 'completed' && (l.ownerId === currentUserId || l.accepted?.some((a:any) => a.applicantId === currentUserId))).map(l => (
                                 <div key={l.id} className="bg-white/5 border border-white/10 p-6 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center group hover:border-[#ff007f]/40 transition-all">
                                    <div className="flex items-center gap-6">
                                       <div className="w-16 h-16 rounded-2xl bg-black border border-white/10 flex items-center justify-center text-3xl opacity-50">{l.title.includes('Magister') ? '🏯' : '🐲'}</div>
                                       <div>
                                    ...
                                    <div className="flex gap-3 mt-4 md:mt-0">
                                       <button onClick={() => { setTargetLobby(l); setIsManageModalOpen(true); }} className="px-6 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Review Terminal</button>
...
// 7. Display Stats in apply modal or profile
content = content.replace(
    '<span>{app.class}</span>',
    `<span>{app.class}</span>\n                                                       <span className="w-1 h-1 rounded-full bg-white/20"></span>\n                                                       <span className="text-yellow-500">{registeredUsers.find(u => u.id === app.applicantId || u.username === app.applicantId)?.stats?.total || 0} Runs</span>`
);

fs.writeFileSync(path, content);
console.log('Advanced Lifecycle & Stats Overhaul Complete');
