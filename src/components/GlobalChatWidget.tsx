...
export default function GlobalChatWidget({ open: controlledOpen, onOpenChange }: Props) {
  const { data: session, status } = useSession();
  const currentUserId = String((session?.user as any)?.id || "");
  const canChat = status === "authenticated" && Boolean(currentUserId);
  ...
  }, [messages, isOpen, replyTo, imagePreview, editingId]);
  ...
  const startEdit = (m: CommunityChatMessage) => {
    setEditingId(m.id);
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg(data?.error || "Could not edit message");
      }
    } catch {
      setErrorMsg("Network error");
    }
  ...
  const handleToggle = () => {
    if (!canChat) {
      setErrorMsg("Sign in with Discord to use Global Chat.");
      return;
    }
    setIsOpen(!isOpen);
  };
  ...
  const renderReactions = (m: CommunityChatMessage) => {
    ...
    return (
      <div className={`flex flex-wrap gap-1 mt-1 ${String(m.userId) === currentUserId ? "justify-end" : "justify-start"}`}>
        {entries.map(([uid, emoji]) => (
          <button
            ...
            className={`text-[11px] px-1.5 py-0.5 rounded-full border transition ${
              uid === currentUserId ? "bg-[#00ffff]/15 border-[#00ffff]/30" : "bg-white/5 border-white/10 hover:bg-white/10"
            }`}
          >
            {emoji}
          </button>
  ...
  const isAdmin = String((session?.user as any)?.role) === "admin" || String((session?.user as any)?.id) === "1497295886223544471";

  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[9997] flex flex-col items-end">
      <AnimatePresence>
        {isOpen && canChat && (
          <motion.div
          ...
          >
            <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between bg-gradient-to-r from-[#00ffff]/10 via-[#8a2be2]/10 to-[#ff007f]/10 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-white">Global Chat</p>
            ...
            <div ref={scrollRef} className={`${isFullscreen ? "flex-1" : "h-64 sm:h-72"} overflow-y-auto px-3 py-3 space-y-3 custom-scrollbar`}>
              {loading && messages.length === 0 ? (
                ...
                messages.map((m) => {
                  const isMine = String(m.userId) === currentUserId;
                  const isEditing = editingId === m.id;
                  return (
                    <div key={m.id} className={`group flex gap-2 ${isMine ? "flex-row-reverse" : ""}`}>
                      <img
                        src={m.userImage}
                        alt=""
                      ...
                      <div className={`max-w-[78%] min-w-0 ${isMine ? "items-end" : "items-start"} flex flex-col`}>
                        {isEditing ? (
                          <div className="w-full rounded-2xl px-3 py-2 bg-white/[0.04] border border-[#00ffff]/40">
                            <textarea
                              value={editingText}
                            ...
                            <div className="flex items-center justify-end gap-1.5 mt-1">
                              <button
                                ...
                                className="px-2 py-1 text-[8px] font-black uppercase tracking-widest text-gray-400 hover:text-white rounded-lg hover:bg-white/10"
                              >
                                Cancel
                              </button>
                              <button
                                ...
                                className="flex items-center gap-1 px-2.5 py-1 text-[8px] font-black uppercase tracking-widest text-black bg-[#00ffff] rounded-lg disabled:opacity-30 hover:opacity-90"
                              >
                          ...
                          >
                            ...
                            {m.edited ? (
                              <span className="block text-[8px] text-gray-500 italic mt-0.5">edited</span>
                            ) : null}
                          </div>
                        )}

                        {!isEditing && renderReactions(m)}
                                ...
                                {(isMine || isAdmin) && (
                                  <>
                                    <button
                                      ...
                                      title="Edit"
                                      onClick={() => startEdit(m)}
                                      className="p-1 rounded-lg bg-white/5 text-gray-400 hover:text-yellow-300 transition"
                                    >
                                    ...
                                    {deleteConfirmId === m.id ? (
                                      <button
                                        ...
                                        className="px-1.5 py-0.5 rounded-lg bg-red-500/20 border border-red-500/40 text-red-300 text-[8px] font-black uppercase tracking-widest"
                                      >
                                        Delete?
                                      </button>
                                    ) : (
                                      <button
                                        ...
                                        className="p-1 rounded-lg bg-white/5 text-gray-400 hover:text-red-400 transition"
                                      >
            ...
            <div className="p-3 border-t border-white/10 space-y-2 shrink-0">
              {errorMsg && (
                <p className="text-center text-[9px] font-bold uppercase tracking-widest text-red-400">{errorMsg}</p>
              )}
              {replyTo ? (
