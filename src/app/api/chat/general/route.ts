...
export async function POST(req: Request) {
  ...
  const { userId, session, profile, handle } = auth;
  const body: any = await req.json().catch(() => ({}));
  const messages: CommunityChatMessage[] = (await getKV(KV_KEY)) || [];

  if (body?.action === "react") {
    const rl = await rateLimitByUser(userId!, "general_chat_react", 60, 60_000);
  ...
  if (body?.action === "edit") {
    const rl = await rateLimitByUser(userId!, "general_chat_edit", 30, 60_000);
    if (!rl.ok) return NextResponse.json({ error: "Slow down." }, { status: 429 });
    ...
    if (String(target.userId) !== String(userId) && !isPrimaryAdmin(userId!, handle)) {
    ...
    messages[idx] = { ...target, text, edited: true };
    await setKV(KV_KEY, messages);
    return NextResponse.json({
      success: true,
      message: enrichReviewWithProfile(messages[idx], profile),
  ...
  if (body?.action === "delete") {
    const rl = await rateLimitByUser(userId!, "general_chat_delete", 30, 60_000);
    if (!rl.ok) return NextResponse.json({ error: "Slow down." }, { status: 429 });
    ...
    const next = [...messages.slice(0, idx), ...messages.slice(idx + 1)];
    await setKV(KV_KEY, next);
    return NextResponse.json({ success: true, messageId });
  }

  const rl = await rateLimitByUser(userId!, "general_chat", 30, 60_000);
