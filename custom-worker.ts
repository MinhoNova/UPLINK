  ...
  async scheduled(_event: ScheduledEvent, env: CloudflareEnv, ctx: ExecutionContext) {
    ctx.waitUntil(
      (async () => {
        ...
        // Refresh Blizzard meta data every 15 minutes
        ...
        if (minute % 15 === 0) {
          try {
            ...
            const [live, ptr] = await Promise.all([
              fetch(`${baseUrl}/api/wow/blizzard-meta?refresh=1`, { signal: AbortSignal.timeout(timeout) }),
              fetch(`${baseUrl}/api/wow/blizzard-meta?ptr=1&refresh=1`, { signal: AbortSignal.timeout(timeout) }),
            ]);
